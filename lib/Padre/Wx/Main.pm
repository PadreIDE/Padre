...
sub new {
    ...
    my $self
        = $class->SUPER::new( undef, -1, 'Padre', $position, $size, $style, );
    ...
    $self->{locale}
        = ( $startup_locale
        ? Padre::Locale::object($startup_locale)
        : Padre::Locale::object() );
...
sub panel_breakpoints {
    ...
    unless ( defined $self->{panel_breakpoints} ) {
        ...
        $self->{panel_breakpoints}
            = Padre::Wx::Panel::Breakpoints->new($self);
    }
...
sub panel_debug_output {
    ...
    unless ( defined $self->{panel_debug_output} ) {
        ...
        $self->{panel_debug_output}
            = Padre::Wx::Panel::DebugOutput->new($self);
    }
...
sub single_instance_address {
    ...
    if (Padre::Constant::WIN32) {
        ...
        # return $address;
        return ( '127.0.0.1', $config->main_singleinstance_port, );
    }
    else {
...
sub single_instance_command {
    ...
    if ( $1 eq 'focus' ) {
        ...
        push @{ $self->{on_close_watchers}->{$line} }, sub {
            ...
            $socket->Write( $buf, length($buf) )
                ;        # XXX length is encoding-sensitive!
    ...
    foreach my $char ( $template =~ /\%(.)/g ) {
        ...
        if ( $char eq 'p' ) {
            ...
            if ( defined $self->ide->{session} ) {
                my ($session)
                    = Padre::DB::Session->select( 'where id = ?',
                    $self->ide->{session},
                    );
...
sub change_locale {
    ...
    unless ( defined $name ) {
        $name = Padre::Locale::system_rfc4646
            || Padre::Locale::last_resort_rfc4646;
    }
...
sub on_run_tdd_tests {
    ...
    my $perl = $self->config->run_perl_cmd
        ; # TODO make this the user selected perl also do it in Padre::Document::Perl::get_command
    unless ($perl) {
    ...
    if ($perl) {
        if ( -e 'Build.PL' ) {
            ...
            $self->error(
                Wx::gettext("No Build.PL nor Makefile.PL nor dist.ini found")
            );
        }
    }
    else {
...
sub on_run_tests {
    ...
    my $filename
        = defined( $document->{file} ) ? $document->{file}->filename : undef;
    unless ($filename) {
        return $self->error(
            Wx::gettext("Current document has no filename") );
    }
...
sub on_run_this_test {
    ...
    unless ( $filename =~ /\.t$/ ) {
        return $self->error(
            Wx::gettext("Current document is not a .t file") );
    }
...
sub run_command {
    ...
    if ( $config->run_use_external_window ) {
       ...
       #gnome-terminal -e "bash -c \"prove -lv t/96_edit_patch.t; exec bash\""
                    system
                        qq($ENV{COLORTERM} -e "bash -c \\\"$cmd; exec bash\\\"" & );
                }
                else {
    ...
    # Start the command
    my $process
        = Wx::Perl::ProcessStream::Process->new( $cmd, "Run $cmd", $self );
...
sub run_document {
    ...
    unless ( $document->can('get_command') ) {
        return $self->error(
            Wx::gettext(
                'No execution mode was defined for this document type')
                . ': '
                . $document->mimetype
        );
    }
    ...
    my $cmd
        = eval { $document->get_command( { $trace ? ( trace => 1 ) : () } ) };
    if ($@) {
        chomp $@;
        $self->error($@);
        return;
...
# Please feel free to fix this
                File::pushd::pushd( $document->project_dir )
                    if -e $document->project_dir;
            ...
            my $ret    = Wx::MessageBox(
                ...
                Wx::gettext("Warning"), $styles, $self,
            );
            if ( $ret == Wx::YES ) {
            SCOPE: {
                    require File::pushd;
...
sub open_session {
    ...
    my $progress = Padre::Wx::Progress->new(
        $self,
        sprintf( Wx::gettext('Opening session %s...'), $session->name, ),
        $#files + 1,
        lazy => 1
    );
...
sub save_session {
    ...
    Padre::DB->do( 'UPDATE session SET last_update=? WHERE id=?',
        {}, time, $session->id );
...
sub prompt {
    ...
    require Padre::Wx::History::TextEntryDialog;
    my $dialog
        = Padre::Wx::History::TextEntryDialog->new( $self, $title, $subtitle,
        $key, );
    if ( $dialog->ShowModal == Wx::ID_CANCEL ) {
        return;
    }
...
sub search_next {
    ...
    $search = $self->{search} = Padre::Search->new(
        ...
        find_regex   => 0,
        find_reverse => 0,
        find_term    => $editor->GetTextRange( $position1, $position2, ),
    );
    return !!$search->search_next($editor);
}
...
sub on_comment_block {
    ...
    my $end_line
        = $editor->LineFromPosition( $selection_start == $selection_end
        ? $selection_end
        : $selection_end - 1 );
    ...
    if ( not defined $comment ) {
        $self->error(
            sprintf(
                Wx::gettext(
                    'Could not determine the comment character for %s document type'
                ),
    ...
    if ( $selection_end > $selection_start ) {
        $editor->SetSelection( $selection_start,
            $selection_end 
                + ( length $document->text_get )
                - $length_before );
    }
    return;
}
...
sub on_autocompletion {
    ...
    if (@words) {
        ...
        $editor->AutoCompSetChooseSingle(
            $config->autocomplete_always ? 0 : 1 );
...
sub on_close_window {
    ...
    if ( $event->CanVeto ) {
        ...
        if ( $self->{command} ) {
            my $ret = Wx::MessageBox(
                Wx::gettext(
                    "You still have a running process. Do you want to kill it and exit?"
                ),
                Wx::gettext("Warning"),
                Wx::YES_NO | Wx::CENTRE,
                $self,
...
SCOPE: {
        ...
        my $lock
            = $self->lock( 'UPDATE', 'DB', 'refresh', 'update_last_session' );
...
# WARNING: This currently only works on local files!
        if ( -d $file_obj->{filename} ) {
            $self->error(
                sprintf( Wx::gettext("Cannot open a directory: %s"), $file )
            );
            return;
        }
    }
    ...
    my $lock
        = $self->lock( 'REFRESH', 'update_last_session', 'refresh_menu' );
    ...
    if ( $document->is_new ) {
        ...
        TRACE( "Adding new file to history: " . $document->filename )
            if DEBUG;
...
# We assume you are opening _one_ file, so newlines in the middle are significant
    ...
    SCOPE: {
            my $filename
                = File::Spec->catfile( $self->ide->{original_cwd}, $text, );
            if ( -f $filename ) {
                push @files, $filename;
            }
        }
        ...
        if ( $current->filename ) {
            my $filename = File::Spec->catfile(
                File::Basename::dirname( $current->filename ), $text, );
            if ( -f $filename ) {
                push @files, $filename;
            }
        }
    ...
    if ( @files > 1 ) {
        ...
        # Pick a file
        my $file
            = $self->single_choice( Wx::gettext('Choose File'), '', [@files],
            );
...
# But I don't think Wx + Motif is in use nowadays
    ...
    $wildcards
        ...
        : Wx::gettext('All Files') . '|*|' . $wildcards;
    my $dialog = Wx::FileDialog->new( $self, Wx::gettext('Open File'),
...
sub reload_all {
    ...
    require Padre::Wx::Progress;
    my $progress
        = Padre::Wx::Progress->new( $self, Wx::gettext('Reload all files'),
        $#pages, lazy => 1 );
...
sub reload_some {
    ...
    require Padre::Wx::Progress;
    my $progress
        = Padre::Wx::Progress->new( $self, Wx::gettext('Reload some'),
        $#reload_pages, lazy => 1 );

SCOPE: {
        my $lock = $self->lock('refresh');
        foreach my $reload_page_no ( 0 .. $#reload_pages ) {
            $progress->update( $reload_page_no,
                ( $reload_page_no + 1 ) . '/' . scalar(@reload_pages) );
...
sub on_save_as {
    ...
    if ( defined $current ) {
        ...
        if (@subpath) {
            my $subdir
                = File::Spec->catdir( $document->project_dir, @subpath, );
            if ( -d $subdir ) {
                $self->{cwd} = $subdir;
            }
        }
...
sub on_save_intuition {
    ...
    unless ( -d $dir ) {
        ...
        if (@$error) {
            $self->error(
                sprintf( Wx::gettext("Failed to create path '%s'"), $dir ) );
            return $self->on_save_as(@_);
        }
    }
...
sub _save_buffer {
    ...
    unless ( $doc->save_file ) {
        $self->error( Wx::gettext("Could not save file: ") . $doc->errstr,
            Wx::gettext("Error"), );
        return;
    }
...
sub close_all {
  ...
  # Do this before closing as the session shouldn't be affected by a close-all
    ...
    require Padre::Wx::Progress;
    my $progress = Padre::Wx::Progress->new( $self, Wx::gettext('Close all'),
        $#pages, lazy => 1 );
...
sub on_close_some {
    ...
    Padre::Wx::Dialog::WindowList->new(
        ...
        buttons =>
            [ [ 'Close selected', sub { $_[0]->main->close_some(@_); } ] ],
    )->show;
}
...
sub close_some {
    ...
    require Padre::Wx::Progress;
    my $progress = Padre::Wx::Progress->new( $self, Wx::gettext('Close some'),
        $#close_pages, lazy => 1 );

SCOPE: {
        my $lock = $self->lock('refresh');
        foreach my $close_page_no ( 0 .. $#close_pages ) {
            $progress->update( $close_page_no,
                ( $close_page_no + 1 ) . '/' . scalar(@close_pages) );
...
sub close_where {
    ...
    my @close = grep { $where->( $notebook->GetPage($_)->{Document} ) }
...
sub delete {
    ...
    if ( !$file->can_delete ) {
        $self->error(
            Wx::gettext('This type of file (URL) is missing delete support.')
        );
        return 1;
    }
    ...
    if ( !$filename ) {
        $self->error(
            Wx::gettext(
                "File was never saved and has no filename - can't delete from disk"
            )
        );
        return 1;
    }
    ...
    my $ret = Wx::MessageBox(
        sprintf(
            Wx::gettext(
                "Do you really want to close and delete %s from disk?"),
            $filename
        ),
        Wx::YES_NO | Wx::CANCEL | Wx::CENTRE,
        $self,
    ...
    TRACE( join ' ', "Deleting ", ref $document, $filename || 'Unknown' )
        if DEBUG;
...
sub on_toggle_lockinterface {
    ...
    # Update setting
    $self->config->apply( 'main_lockinterface',
...
sub on_stc_style_needed {
    ...
    if ( $document->can('colorize') ) {
        ...
        $document->{_text} = $text;
        $document->colorize( $current->editor->GetEndStyled,
            $event->GetPosition );
    }
}
...
sub on_doc_stats {
    ...
    if ( not $doc ) {
        $self->message( Wx::gettext('No file is open'),
            Wx::gettext('Stats') );
        return;
    }
...
sub on_tab_and_space {
    ...
    require Padre::Wx::History::TextEntryDialog;
    my $dialog
        = Padre::Wx::History::TextEntryDialog->new( $self,
...
sub timer_check_overwrite {
    ...
    my $doc        = $self->current->document or return;
    my $file_state = $doc
        ->has_changed_on_disk;    # 1 = updated, 0 = unchanged, -1 = deleted
...
sub on_duplicate {
    my $self = shift;
    my $document = $self->current->document or return;
    return $self->new_document_from_string( $document->text_get,
        $document->mimetype, );
}
...
sub start_perl5_module {
    ...
    require Padre::Template;
    my $code
        = Padre::Template->render( 'perl5/module.pm', module => $module, );
...
sub change_highlighter {
    ...
    foreach my $editor ( $self->editors ) {
        ...
        my $filename
            ...
            : undef;
        TRACE( "Set highlighter to to $module for $document in file "
                . ( $filename || '' ) )
            if DEBUG;
...
sub key_up {
    ...
    if ( $mod == Wx::MOD_CMD ) {    # Ctrl
        ...
        if ( $code == Wx::K_TAB ) {
            &{ $self->ide->actions->{'window.next_file'}->menu_event }( $self,
                $event );
        }
    }
        ...
        if ( $code == Wx::K_TAB ) {
            ...
            &{ $self->ide->actions->{'window.previous_file'}->menu_event }(
                $self, $event );
        }
    }
        ...
        &{ $self->ide->actions->{'view.close_panel'}->menu_event }( $self,
            $event );
    }
...
sub show_as_numbers {
    ...
    unless ($text) {
        $self->message(
            Wx::gettext('Need to select text in order to translate to hex') );
        return;
    }
...
sub _filter_tool_run {
    ...
    {
        $self->error(
            sprintf( Wx::gettext("Error running filter tool:\n%s"), $! ) );
        return;
    }
...
sub encode {
    ...
    $self->message(
        sprintf( Wx::gettext('Document encoded to (%s)'), $charset, ) );
    return;
}

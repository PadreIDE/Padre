...
sub load_files {
    ...
    if ( defined $ide->opts->{session} ) {
        ...
        my ($session) =
          Padre::DB::Session->select( 'where name = ?', $ide->opts->{session},
          );
...
sub _timer_post_init {
    ...
    $timer->Start( $self->ide->config->update_file_from_disk_interval * SECONDS,
        0 );
...
sub run_command {
    ...
    # Start the command
    $self->{command} =
      Wx::Perl::ProcessStream->OpenProcess( $cmd, 'MyName1', $self );
...
sub prompt {
    ...
    require Padre::Wx::History::TextEntryDialog;
    my $dialog =
      Padre::Wx::History::TextEntryDialog->new( $self, $title, $subtitle, $key,
      );
    if ( $dialog->ShowModal == Wx::wxID_CANCEL ) {
        return;
    }
...
sub on_autocompletion {
    ...
    if ( $length =~ /\D/ ) {
        Wx::MessageBox( $length, Wx::gettext("Autocompletion error"), Wx::wxOK,
        );
    }
    if (@words) {
...
#atm, we assume you are opening _one_ file, so newlines in the middle are significant
    ...
    unless (@files) {    # TODO: and if we are in a Perl environment
        ...
        $module .= ".pm";
        my $filename =
          File::Spec->catfile( $self->ide->{original_cwd}, $module, );
        if ( -e $filename ) {
            push @files, $filename;
        }
        else {
            ...
            my $filename =
              File::Spec->catfile( $self->current->document->project_dir,
                'lib', $module, );
            if ( -e $filename ) {
                push @files, $filename;
            }
...
sub on_save_as {
    ...
    while (1) {
        my $dialog = Wx::FileDialog->new( $self, Wx::gettext("Save file as..."),
        ...
        if ( -e $path ) {
            my $response =
              Wx::MessageBox( Wx::gettext("File already exists. Overwrite it?"),
...
"File changed on disk since last saved. Do you want to overwrite it?"
    ...
    unless ( $doc->save_file ) {
        Wx::MessageBox( Wx::gettext("Could not save file: ") . $doc->errstr,
            Wx::gettext("Error"), Wx::wxOK, $self, );
        return;
    }
...
sub on_diff {
    ...
    unless ($file) {
        return $self->error(
            Wx::gettext("Cannot diff if file was never saved") );
    }
...
sub on_toggle_right_margin {
    ...
    foreach my $editor ( $self->editors ) {
        ...
        $editor->SetEdgeMode(
            $enabled ? Wx::wxSTC_EDGE_LINE : Wx::wxSTC_EDGE_NONE );
    }
...
sub on_toggle_indentation_guide {
    ...
    $self->config->set( 'editor_indentationguides',
        $self->menu->view->{indentation_guide}->IsChecked ? 1 : 0,
    );

    foreach my $editor ( $self->editors ) {
        $editor->SetIndentationGuides(
            $self->config->editor_indentationguides );
    }
...
sub on_toggle_statusbar {
    ...
    # Update the configuration
    $self->config->set( 'main_statusbar',
...
sub on_toggle_lockinterface {
    ...
    $config->apply( 'main_lockinterface',
...
sub on_doc_stats {
    ...
    my @messages = (
        ...
        sprintf(
            Wx::gettext("Chars without spaces: %s"), $chars_without_space
        ),
...
sub on_delete_ending_space {
    ...
    my $src      = $current->text;
    my $code =
      ( defined($src) && length($src) > 0 ) ? $src : $document->text_get;
...
sub timer_check_overwrite {
    ...
    my $doc = $self->current->document or return;
    my $file_state =
      $doc->has_changed_on_disk;    # 1 = updated, 0 = unchanged, -1 = deleted
...
'File has been deleted on disk, do you want to CLEAR the editor window?'
        ...
        $Text = Wx::gettext(
            "File changed on disk since last saved. Do you want to reload it?");
    }
...
sub on_new_from_template {
    ...
    my $editor = $self->current->editor or return;
    my $file = File::Spec->catfile( Padre::Util::sharedir('templates'),
        "template.$extension" );
    ...
    if ( $editor->insert_from_file($file) ) {
        ...
        $self->message(
            sprintf( Wx::gettext("Error loading template file '%s'"), $file ) );
    }
    return;
}
...
sub change_highlighter {
    ...
    foreach my $editor ( $self->editors ) {
        ...
        Padre::Util::debug(
            "Set highlighter to to $module for $document in file "
              . ( $document->filename || '' ) );

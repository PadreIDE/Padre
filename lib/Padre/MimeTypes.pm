...
sub _initialize {
 ...
 # array ref of objects with value and mime_type fields that have the raw values
    ...
    __PACKAGE__->add_highlighter( 'stc', _T('Scintilla'),
    ...
    __PACKAGE__->add_highlighter(
        ...
        _T('PPI Standard'),
        _T(
'Hopefully faster than the PPI Traditional. Big file will fall back to Scintilla highlighter.'
        )
    );

    __PACKAGE__->add_highlighter_to_mime_type( 'application/x-perl',
...
sub add_mime_class {
    ...
    if ( not $MIME_TYPES{$mime} ) {
        Padre::Current->main->error(
            sprintf(
                Wx::gettext(
                    "Mime type was not supported when %s(%s) was called"),
                'add_mime_class',
                $mime
            )
        );
    ...
    if ( $MIME_TYPES{$mime}{class} ) {
        Padre::Current->main->error(
            sprintf(
                Wx::gettext(
                    "Mime type already had a class '%s' when %s(%s) was called"
                ),
...
sub remove_mime_class {
    ...
    if ( not $MIME_TYPES{$mime} ) {
        Padre::Current->main->error(
            sprintf(
                Wx::gettext(
                    "Mime type is not supported when %s(%s) was called"),
                'remove_mime_class',
                $mime
            )
        );
    ...
    if ( not $MIME_TYPES{$mime}{class} ) {
        Padre::Current->main->error(
            sprintf(
                Wx::gettext(
"Mime type did not have a class entry when %s(%s) was called"
                ),
                'remove_mime_class',
                $mime
            )
...
sub get_highlighter_explanation {
    ...
    my ($highlighter) =
      grep { $AVAILABLE_HIGHLIGHTERS{$_}{name} eq $name }
      keys %AVAILABLE_HIGHLIGHTERS;
    if ( not $highlighter ) {
...
# we need to handle this situation
    ...
    return ''
      if not $AVAILABLE_HIGHLIGHTERS{$highlighter};    # avoid autovivification
...
sub change_highlighters {
    ...
    my %highlighters =
      map { $AVAILABLE_HIGHLIGHTERS{$_}{name} => $_ }
      keys %AVAILABLE_HIGHLIGHTERS;

    foreach my $mime_type_name ( keys %$changed_highlighters ) {
        my $mime_type = $mtn{$mime_type_name};    # get mime_type from name
        my $highlighter =
          $highlighters{ $changed_highlighters
              ->{$mime_type_name} };              # get highlighter from name
...
sub get_current_highlighter_names {
    ...
    foreach my $mime_type ( keys %MIME_TYPES ) {
        $MT{ $self->get_mime_type_name($mime_type) } =
          $self->get_highlighter_name(
            $MIME_TYPES{$mime_type}{current_highlighter} );
    }
    return \%MT;
}
...
sub get_mime_types {
    return [ sort { lc $MIME_TYPES{$a}{name} cmp lc $MIME_TYPES{$b}{name} }
          keys %MIME_TYPES ];
}
...
sub get_highlighters_of_mime_type {
    ...
    my @names =
      ...
      sort keys %{ $MIME_TYPES{$mime_type}{highlighters} };
    return \@names;
}
...
sub get_highlighters_of_mime_type_name {
    ...
    my ($mime_type) =
      grep { $MIME_TYPES{$_}{name} eq $mime_type_name } keys %MIME_TYPES;
    if ( not $mime_type ) {
        warn
"Could not find the MIME type of the display name '$mime_type_name'\n";
...
sub _guess_mimetype {
    warn join( ',', caller )
      . ' called MimeTypes::_guess_mimetype which is depreached, use ::guess_mimetype!';
...
sub get_extensions_by_mime_type {
    ...
    while ( my ( $key, $value ) = each(%EXT_MIME) ) {
        ...
        $value = 'application/x-perl'
          ...
          ;    # assume the hash holds a code ref for all perl extensions.

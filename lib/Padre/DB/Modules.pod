...
Padre::DB::Modules - Padre::DB class for the modules table
...
=head2 select
  ...
  my @list = Padre::DB::Modules->select;
  
  # Get a subset of objects in scalar context
  my $array_ref = Padre::DB::Modules->select(
...
Returns a list of B<Padre::DB::Modules> objects when called in list context, or a
reference to an ARRAY of B<Padre::DB::Modules> objects when called in scalar context.
...
=head2 count
  ...
  my $rows = Padre::DB::Modules->count;
  
  # How many objects 
  my $small = Padre::DB::Modules->count(
      'where id > ?',
      1000,
  );
...
Returns a new L<Padre::DB::Modules> object.

=head2 create

  my $object = Padre::DB::Modules->create(
...
L<Padre::DB::Modules> object, inserts the appropriate row into the L<modules>
...
Returns a new L<modules> object, or throws an exception on error,
...
=head2 delete
  ...
  # Delete multiple rows from the modules table
  Padre::DB::Modules->delete('where id > ?', 1000);

The C<delete> method can be used in a class form and an instance form.

When used on an existing B<Padre::DB::Modules> instance, the C<delete> method
removes that specific instance from the C<modules>, leaving
...
phrase to be added after the C<DELETE FROM modules> section
...
=head2 truncate
  ...
  # Delete all records in the modules table
...
following SQL command.
  ...
  CREATE TABLE modules (id INTEGER PRIMARY KEY, name VARCHAR(100))

=head1 SUPPORT

Padre::DB::Modules is part of the L<Padre::DB> API.
